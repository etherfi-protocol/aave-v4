...
import {AaveV4DeployMapleCorrelatedSpoke} from 'scripts/deploy/AaveV4DeployCorrelatedSpoke.s.sol';

contract AaveV4DeployMapleCorrelatedSpokeTest is Test {
  AaveV4DeployMapleCorrelatedSpoke internal _script;

  function setUp() public {
    vm.createSelectFork(vm.rpcUrl('mainnet'), 25092080);
    _script = new AaveV4DeployMapleCorrelatedSpoke();
  }
  ...
  function test_spokeSaltMatchesOrchestrationFormula_fuzz(address deployer) public view {
    ...
    bytes32 expected = keccak256(abi.encode(expectedRoot, 'spoke', 'MAPLE_CORRELATED_SPOKE'));
